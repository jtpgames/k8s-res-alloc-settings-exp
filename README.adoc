...
----
podman run --volume "$PWD":/mnt/host --workdir /mnt/host -it --name experiment_runner experiment_runner/ubuntu:24.04 bash
----
...
----
doctl kubernetes cluster create k8s-experiments-cluster --version 1.33.1-do.3 --count 2 --size s-4vcpu-8gb --verbose
----
